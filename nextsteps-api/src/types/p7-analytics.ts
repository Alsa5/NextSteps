...
export interface CohortComparisonResponse {
  batchA: { batchId: string; metrics: CohortMetrics };
  batchB: { batchId: string; metrics: CohortMetrics };
  metrics: {
    feedbackCompletion: { batchA: number; batchB: number; delta: number };
    quizAverage: { batchA: number; batchB: number; delta: number };
    readinessScore: { batchA: number; batchB: number; delta: number };
  };
  insights: string[];
}
