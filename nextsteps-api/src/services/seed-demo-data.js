...
const curriculumByBatch = {
    [BATCH_ID]: [
        {
            topic: 'Async JavaScript patterns',
            confidencePercent: 82,
            rationale: 'Confusion spikes in 3 sessions across batches; pulse feedback cites event-loop clarity.',
...
export const seedDemoData = async (db) => {
    ...
    for (const account of DEV_USERS) {
        ...
        const existing = await db
            .collection(NEXTSTEPS_COLLECTIONS.USERS)
            .findOne({ email });
        if (existing) {
            await db.collection(NEXTSTEPS_COLLECTIONS.USERS).updateOne({ email }, {
                $set: {
            ...
            await db.collection(NEXTSTEPS_COLLECTIONS.USERS).insertOne({
                _id: randomUUID(),
                email,
                fullName: account.fullName,
                role: account.role,
    ...
    for (const entry of traineeEntries) {
        ...
        await db.collection(NEXTSTEPS_COLLECTIONS.TRAINEE_REGISTRY).updateOne({ email }, {
            $set: {
                email,
    ...
    for (const session of seedSessions) {
        await db.collection(NEXTSTEPS_COLLECTIONS.SESSIONS).updateOne({ id: session.id }, { $set: session }, { upsert: true });
    }
    for (const transcript of seedSessionTranscripts) {
        await db.collection(NEXTSTEPS_COLLECTIONS.SESSION_TRANSCRIPTS).updateOne({ sessionId: transcript.sessionId }, { $set: transcript }, { upsert: true });
    }
    const workerTranscriptAt = new Date('2025-04-28T11:00:00.000Z');
    await db.collection(NEXTSTEPS_COLLECTIONS.TRANSCRIPTS).updateOne({ sessionId: 'ses-001' }, {
        $set: {
            sessionId: 'ses-001',
    ...
    await db.collection(NEXTSTEPS_COLLECTIONS.BATCH_FEEDBACK_STATE).updateOne({ batchId: BATCH_ID }, {
        $set: {
            batchId: BATCH_ID,
            trainerId: TRAINER_ID,
    ...
    for (const sub of seedFeedbackSubmissions) {
        await db.collection(NEXTSTEPS_COLLECTIONS.FEEDBACK_SUBMISSIONS).updateOne({ batchId: BATCH_ID, sessionId: sub.sessionId, maverickId: sub.maverickId }, { $set: { batchId: BATCH_ID, ...sub } }, { upsert: true });
    }
    for (const [maverickId, batchId] of Object.entries(maverickBatches)) {
        await db.collection(NEXTSTEPS_COLLECTIONS.MAVERICK_BATCH_MEMBERSHIPS).updateOne({ maverickId }, { $set: { maverickId, batchId } }, { upsert: true });
    }
    for (const quiz of seedQuizzes) {
        await db.collection(NEXTSTEPS_COLLECTIONS.QUIZZES).updateOne({ id: quiz.id }, { $set: quiz }, { upsert: true });
    }
    ...
    for (const note of seedNotifications) {
        await db.collection(NEXTSTEPS_COLLECTIONS.NOTIFICATIONS).updateOne({ id: note.id }, {
            $set: { ...note, read: false, createdAt: now },
        }, { upsert: true });
    }
    await db.collection(NEXTSTEPS_COLLECTIONS.AUDIT_LOGS).updateOne({ id: 'audit-seed-ld-dashboard' }, {
        $set: {
    ...
    for (const [batchId, metrics] of Object.entries(BATCH_METRICS)) {
        await db.collection(NEXTSTEPS_COLLECTIONS.P7_BATCH_METRICS).updateOne({ batchId }, { $set: { batchId, metrics } }, { upsert: true });
    }
    for (const [batchId, recommendations] of Object.entries(curriculumByBatch)) {
        await db.collection(NEXTSTEPS_COLLECTIONS.P7_CURRICULUM_RECOMMENDATIONS).updateOne({ batchId }, { $set: { batchId, recommendations } }, { upsert: true });
    }
    for (const [batchId, data] of Object.entries(topPerformersByBatch)) {
        await db.collection(NEXTSTEPS_COLLECTIONS.P7_TOP_PERFORMERS).updateOne({ batchId }, { $set: { batchId, ...data } }, { upsert: true });
    }
    for (const [batchId, data] of Object.entries(reminderTimingsByBatch)) {
        await db.collection(NEXTSTEPS_COLLECTIONS.P7_REMINDER_TIMINGS).updateOne({ batchId }, { $set: { batchId, ...data } }, { upsert: true });
    }
