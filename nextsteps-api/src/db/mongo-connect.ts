...
export const isRunningOnAzureAppService = (): boolean =>
  Boolean(process.env.WEBSITE_INSTANCE_ID ?? process.env.WEBSITE_SITE_NAME);
...
export const validateMongoUri = (uri: string): void => {
  ...
  throw new Error(
    'MONGODB_URI is still an unresolved Key Vault reference (@Microsoft.KeyVault…). ' +
