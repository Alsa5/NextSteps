...
function iterate(startPoitn, deg, iteration) {
  ...
  var size = lenght / 30;
  var lineEnding = new point(
    lineBeginning.x + lenght * Math.cos(deg + andleOfExpL),
    lineBeginning.y + lenght * Math.sin(deg + andleOfExpL)
  );
  var lineEnding2 = new point(
    lineBeginning.x + lenght * Math.cos(deg - andleOfExpR),
    lineBeginning.y + lenght * Math.sin(deg - andleOfExpR)
  );
...
function animate() {
  ...
  colorId = 0;
  andleOfExpR = getDir(
    new point(0, Math.sin(param / 20) * 8),
    new point(Math.abs(mouse.x), mouse.y)
  );
  andleOfExpL = getDir(
    new point(0, Math.sin(param / 10) * 10),
    new point(Math.abs(mouse.x), mouse.y)
  );
