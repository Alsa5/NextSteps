...
export const DEFAULT_GLOW_COLOR = '123, 92, 245'
...
export const ParticleCard = forwardRef(function ParticleCard(
  {
    children,
    className = '',
  ...
  const mergedStyle = {
    ...
    position: 'relative',
    overflow: 'hidden',
    ...(glowColor ? { '--magic-glow-rgb': glowColor } : {}),
  }

  return (
    <div ref={cardRef} className={`${className} particle-container`.trim()} style={mergedStyle}>
      {children}
    </div>
  )
})
...
export function MagicSpotlight({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      const cards = gridRef.current.querySelectorAll('.app-magic-card')
    ...
    return () => {
      ...
      gridRef.current?.querySelectorAll('.app-magic-card').forEach((card) => {
        card.style.setProperty('--glow-intensity', '0')
      })
    }
...
const BentoCardGrid = ({ children, gridRef }) => (
  <div className="card-grid bento-section magic-bento-demo-grid" ref={gridRef}>
    {children}
  </div>
)

/** Demo grid from React Bits — optional showcase */
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          const baseClassName = `magic-bento-card magic-bento-demo-card app-magic-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''}`
          const cardProps = {
